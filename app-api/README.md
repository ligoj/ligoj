...
```
java -Xmx1024M -Duser.timezone=UTC -Djpa.hbm2ddl=none -Dligoj.plugin.enabled=false -Djdbc.host=ligoj-db -jar target/app-api-3.0.2.war
```

# Build Docker image

```
docker build -t ligoj/ligoj-api:3.0.2 .
```
...
By default, the location is "https://oss.sonatype.org/service/local/artifact/maven/redirect?r=public&g=org.ligoj.app&a=app-api&v=3.0.2&p=war"
In case of a custom build you can specify its remote or local location.

Staged OSS build from Sonatype

```
docker build -t ligoj/ligoj-api:3.0.2 --build-arg WAR="https://oss.sonatype.org/service/local/repositories/orgligoj-1087/content/org/ligoj/app/app-api/3.0.2/app-api-3.0.2.war" .
```

Private remote build

```
docker build -t ligoj/ligoj-api:3.0.2 --build-arg WAR="https://storage.company.com/releases/app-api-3.0.2.war" .
```

Reuse the local maven package

```
docker build -t ligoj/ligoj-api:3.0.2 --build-arg WAR="target/app-api-3.0.2.war" .
```
...
docker run --rm -it \
 --name "ligoj-api" \
 ligoj/ligoj-api:3.0.2 bash -c "apt-get install -y mysql-client && mysql -h 192.168.4.138 --user=ligoj --password=ligoj ligoj"
```
