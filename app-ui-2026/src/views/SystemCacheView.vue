...
<template>
  <div class="caches">
    ...
    <div class="stats">
      <div v-for="(s, i) in stats" :key="s.key" class="stat" :style="{ '--c': s.color, 'animation-delay': (i * 50) + 'ms' }">
        <div class="stop">
          ...
          <div class="sbody"><div class="snum">{{ s.value }}<small v-if="s.unit">{{ s.unit }}</small></div><div class="slabel">{{ s.label }}</div></div>
        </div>
    ...
    <VibrantDataTable :headers="headers" :items="paged" :items-length="filtered.length" :loading="loading" item-value="id" default-sort="id"
      :empty-text="t('common.noData')" @update:options="onOptions">
      <template #cell.id="{ item }">
        <div class="avatar-cell">
          <span class="cglyph"><v-icon size="18">mdi-database-outline</v-icon></span>
          <code class="cname">{{ item.id }}</code>
        </div>
      </template>
      <template #cell.size="{ item }"><span class="num">{{ (item.size ?? 0).toLocaleString() }}</span></template>
      <template #cell.hitCount="{ item }">
        <span class="rate">
          <span class="num">{{ (item.hitCount ?? 0).toLocaleString() }}</span>
      ...
      <template #cell.missCount="{ item }">
        <span class="rate">
          <span class="num">{{ (item.missCount ?? 0).toLocaleString() }}</span>
      ...
      <template #cell.averageGetTime="{ item }"><span class="num">{{ item.averageGetTime ?? '—' }}</span></template>
...
const stats = computed(() => {
  ...
  const times = items.value.map((c) => c.averageGetTime).filter((v) => typeof v === 'number')
  const avgGet = times.length ? (times.reduce((s, v) => s + v, 0) / times.length) : 0
  return [
    { key: 'caches', label: t('system.cache.statCaches'), icon: 'mdi-database-outline', color: 'rgb(var(--v-theme-secondary))', value: items.value.length, pct: 100 },
    { key: 'entries', label: t('system.cache.statEntries'), icon: 'mdi-counter', color: '#2f6df6', value: entries.toLocaleString(), pct: 100 },
    { key: 'hit', label: t('system.cache.statHitRate'), icon: 'mdi-check-circle-outline', color: '#1d9d63', value: hitRate, unit: '%', pct: hitRate },
    { key: 'avg', label: t('system.cache.statAvgGet'), icon: 'mdi-timer-outline', color: '#8b5cf6', value: avgGet ? avgGet.toFixed(2) : '0', unit: ' ms', pct: Math.min(100, avgGet * 20) },
  ]
})
