...
# User section
```
docker build -t ligoj-api:1.6.1 --build-arg VERSION=1.6.1 app-api
docker run -d --name ligoj-api --link ligoj-db:db ligoj-api:1.6.1
docker build -t ligoj-ui:1.6.1 --build-arg VERSION=1.6.1 app-ui
...
```
docker run -d --name ligoj-api --link ligoj-db:db -v ~/.ligoj:/usr/local/ligoj ligoj-api:1.6.1
```
# Dev section
