...
class CacheBustingFilterTest {
	...
	void testIgnorePragmaResponseWrapper() {
		...
		verify(response, atLeastOnce()).setHeader(HttpHeaders.EXPIRES, "Value1");
		verify(response, never()).setHeader(HttpHeaders.PRAGMA, "Value2");
		verify(response, atLeastOnce()).addHeader(HttpHeaders.EXPIRES, "Value1");
		verify(response, never()).addHeader(HttpHeaders.PRAGMA, "Value2");
	}
	...
	void test204() throws ServletException, IOException {
		...
		newFilter(response);
		verify(response, never()).setHeader(HttpHeaders.PRAGMA, "no-cache");
	}
	...
	void test200Expiration() throws ServletException, IOException {
		...
		newFilter(response);
		verify(response).setHeader(HttpHeaders.CACHE_CONTROL, "public, max-age=1");
	}
	...
	void test200NoExpiration() throws ServletException, IOException {
		...
		when(response.getStatus()).thenReturn(200);
		final var request = mock(HttpServletRequest.class);
		new CacheBustingFilter(0).doFilter(request, response, mock(FilterChain.class));
	...
	void test200VersionedOnlyWithoutVersion() throws ServletException, IOException {
		...
		when(response.getStatus()).thenReturn(200);
		final var request = mock(HttpServletRequest.class);
		new CacheBustingFilter(31556926, true).doFilter(request, response, mock(FilterChain.class));
		verify(response).setHeader(HttpHeaders.CACHE_CONTROL, "no-cache");
	}
	...
	void test200VersionedOnlyWithVersion() throws ServletException, IOException {
		...
		when(response.getStatus()).thenReturn(200);
		final var request = mock(HttpServletRequest.class);
		when(request.getParameter("v")).thenReturn("1B2M2Y8AsgTpgAmY7PhCfg==");
		new CacheBustingFilter(31556926, true).doFilter(request, response, mock(FilterChain.class));
		verify(response).setHeader(HttpHeaders.CACHE_CONTROL, "public, max-age=31556926, immutable");
	}
}