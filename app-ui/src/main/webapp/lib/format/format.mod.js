define(['jquery', 'i18n!format/nls/format-messages', 'moment.mod'], function ($, messages) {
	formatManager = {
		 ...
		 * @param {string} format    The format used to build the final value accepting an array having this structure : [ value, power, unit].
		 *                           For sample : [1.23, 'M','£'].
		 ...
		 * @return {string}         Human readable string
		 */
		formatCost: function (bytes, digits, currency, clazz) {
			return formatManager.formatUnit(bytes, digits, ['', 'K', 'M', 'B', 'T'], currency, 1000, messages['format-cost'], clazz);
		},
