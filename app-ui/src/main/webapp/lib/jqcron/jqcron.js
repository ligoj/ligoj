define(['jquery'], function($) {
	...
	var jqCronDefaultSettings = {
		...
		toName: {
			...
			'month' : ['JAN','JAN','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC']
		},
	...
	function jqCron(settings) {
		...
		var _$blockSECS   = $('<span class="jqCron-secs"></span>');
		...
		var _selectors    = [];
		var _selectorPeriod, _selectorMins, _selectorSecs, _selectorTimeH, _selectorTimeM, _selectorTimeS, _selectorDow, _selectorDom, _selectorMonth;
		...
		function newSelector($block, multiple, type){
			...
			selector.$.on('selector:open', function(){
			...
			selector.$.on('selector:change', function(){
				...
				if(settings.multiple_secs == settings.multiple_time_seconds) {
					if(selector == _selectorSecs) {
						boundChanged = _selectorTimeS.setValue(_selectorSecs.getValue());
					}
					else if(selector == _selectorTimeS) {
						boundChanged = _selectorSecs.setValue(_selectorTimeS.getValue());
					}
				}
		...
		this.getCron = function(){
			...
			if(period == 'minute' || period == 'hour') {
				items[0] = _selectorSecs.getCronValue();
				if(period == 'hour') {
					items[1] = _selectorMins.getCronValue();
				}
			} else {
				if(period == 'day' || period == 'week' || period == 'month' || period == 'year') {
					items[0] = _selectorTimeS.getCronValue();
		...
		this.setCron = function(str) {
			...
			try {
				...
				if(mask == '******') {									// Second
					_selectorPeriod.setValue('second');
				}
				else if(mask.substring(1, mask.length) == '*****') {	// Minute
					_selectorPeriod.setValue('minute');
					_selectorSecs.setCronValue(items[0]);
					_selectorTimeS.setCronValue(items[0]);
				}
				else if(mask.substring(1, mask.length) == '-****') {	// Hour
				...
				else if(mask.substring(3, mask.length) == '**-' || mask.substring(3, mask.length) == '-*-') {			// 4 possibilities
		...
		this.init = function(){
			...
			_selectorPeriod.$.on('selector:change', function(e, value){
				_$blockDOM.hide();
				_$blockMONTH.hide();
				_$blockMINS.hide();
				_$blockSECS.hide();
			...
			// SECS  (seconds)
			_$blockSECS.append(_self.getText('text_secs'));
			_selectorSecs = newSelector(_$blockSECS, settings.multiple_secs, 'seconds');
			for(i=0, list=settings.minutes; i<list.length; i++){
				_selectorSecs.add(list[i], list[i]);
			}
			...
			_selectorTimeS = newSelector(_$blockTIME, settings.multiple_time_seconds, 'time_seconds');
			for(i=0, list=settings.minutes; i<list.length; i++){
				_selectorTimeS.add(list[i], list[i]);
			}
	...
	function jqCronSelector(_cron, _$block, _multiple, _type){
		...
		var toName = _cron.getSettings().toName[_type];
		...
		this.getCronValue = function(){
			...
			var cron = [toName ? toName[_value[0]-1] : _value[0]], i, s = _value[0], c = _value[0], n = _value.length;
			for(i=1; i<n; i++) {
				if(_value[i] === c+1) {
					c = _value[i];
					cron[cron.length-1] = toName ? toName[s-1] + '-' + toName[c-1] : (s+'-'+c);
				}
				else {
					s = c = _value[i];
					cron.push(_type === toName ? toName[c-1] : c);
				}
			}
			return cron.join(',');
		};
		
		this.toIndex = function(nameOrIndex) {
			if (toName) {
				for(var index = 0; index<toName.length; index++) {
					if (toName[index] == nameOrIndex) {
						return index + 1;
					}
				}
			}
		...
		this.setCronValue = function(str) {
			var values = [], m ,i, n, s, e, inc;
			if(str !== '*') {
				while(str != '') {
					// test "*/n" expression
					m = str.match(/^\*\/([0-9A-Z]+),?/);
					if(m && m.length == 2) {
						inc = this.toIndex(m[1])|1;
						for(i=0; i<=59; i+=inc) {
							values.push(i);
						}
						str = str.replace(m[0], '');
					...
					m = str.match(/^([0-9A-Z]+)-([0-9A-Z]+)\/([0-9A-Z]+),?/);
					if(m && m.length == 4) {
						s = this.toIndex(m[1])|0;
						e = this.toIndex(m[2])|0;
						inc = this.toIndex(m[3])|1;
						for(i=s; i<=e; i+=inc) {
							values.push(i);
						}
						str = str.replace(m[0], '');
					...
					m = str.match(/^([0-9A-Z]+),?/);
					if(m && m.length == 2) {
						values.push(this.toIndex(m[1])|0);
						str = str.replace(m[0], '');
						continue;
					}
