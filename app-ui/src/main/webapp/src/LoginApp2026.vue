...
.login-bg {
  --bg: #f4f1ec; --surface: #fff; --border: #e9e3d8; --border-2: #ddd5c6;
  --ink: #1c1a17; --ink-2: #5a554c; --ink-3: #8d877b; --ink-4: #b4ad9f;
  --btn1: #ff9436; --btn2: #ff5a52; --primary: #27348a;
  --ok: #1d9d63; --err: #df4d42; --info: #2f6df6; --warn: #e0901a;
...
.login-bg::before, .login-bg::after {
  content: ""; position: absolute; border-radius: 50%; filter: blur(72px); opacity: .45; z-index: 0; pointer-events: none;
}
.login-bg::before { width: 440px; height: 440px; background: var(--btn1); top: -150px; left: -110px; animation: blob1 15s ease-in-out infinite; }
...
.alert-info { background: color-mix(in srgb, var(--info) 10%, var(--surface)); color: #1b4fd6; border-color: color-mix(in srgb, var(--info) 25%, transparent); }
...
input[type="text"], input[type="password"], input[type="email"] {
  ...
  border: 1px solid var(--border); border-radius: 11px; background: #fdfcfa; color: var(--ink);
  transition: border-color .15s, box-shadow .15s, background .15s; font-family: var(--sys);
}
input:focus { outline: none; border-color: var(--btn1); box-shadow: 0 0 0 4px rgba(255,148,54,.15); background: var(--surface); }
input:disabled, input[readonly] { background: #f3efe7; color: var(--ink-3); }
...
.captcha-img { border: 1px solid var(--border); border-radius: 9px; height: 42px; min-width: 150px; cursor: pointer; }
...
.icon-btn:hover { background: #faf7f1; }

.submit {
  position: relative; overflow: hidden;
  width: 100%; padding: 13px; font-family: var(--font); font-size: 1rem; font-weight: 800; color: #fff;
  background: linear-gradient(135deg, var(--btn1), var(--btn2)); border: none; border-radius: 12px; cursor: pointer;
  display: inline-flex; align-items: center; justify-content: center; gap: 8px;
  box-shadow: 0 14px 28px -12px rgba(255,90,82,.7); transition: filter .15s; margin-top: 4px;
}
...
.link { background: none; border: none; color: var(--btn2); font-family: var(--sys); font-weight: 700; font-size: .9rem; cursor: pointer; padding: 4px 8px; }
