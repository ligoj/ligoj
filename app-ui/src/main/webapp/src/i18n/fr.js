export default {
  // Navigation
  'nav.home': 'Accueil',
  'nav.dashboard': 'Tableau de bord',
  'nav.identity': 'Identité',
  'nav.users': 'Utilisateurs',
  'nav.groups': 'Groupes',
  'nav.companies': 'Entités',
  'nav.delegates': 'Délégués',
  'nav.projects': 'Projets',
  'nav.system': 'Administration',
  'nav.information': 'Informations',
  'nav.configuration': 'Configuration',
  'nav.hooks': 'Hooks',
  'nav.roles': 'Rôles',
  'nav.plugins': 'Plugins',
  'nav.nodes': 'Nœuds',
  'nav.cache': 'Cache',
  'nav.bench': 'Bench',
  'nav.tasks': 'Tâches',
  'nav.systemUsers': 'Utilisateurs système',
  'nav.profile': 'Profil',
  'node.iconMissing': 'Plugin indisponible',
  'nav.about': 'À propos',
  'nav.logout': 'Déconnexion',
  'nav.refresh': 'Actualiser',
  'nav.api': 'API',
  'nav.apiExplorer': 'Explorateur',
  'nav.apiTokens': 'Clés API',

  // Common
  'common.search': 'Rechercher',
  'common.save': 'Enregistrer',
  'common.create': 'Créer',
  'common.cancel': 'Annuler',
  'common.delete': 'Supprimer',
  'common.remove': 'Retirer',
  'common.edit': 'Modifier',
  'common.view': 'Voir',
  'common.add': 'Ajouter',
  'common.close': 'Fermer',
  'common.actions': 'Actions',
  'common.alreadyExists': 'Existe déjà',
  'common.name': 'Nom',
  'common.description': 'Description',
  'common.status': 'Statut',
  'common.loading': 'Chargement...',
  'common.loadError': 'La requête a échoué. Veuillez réessayer.',
  'common.required': 'Ce champ est requis',
  'common.positive': 'Doit être supérieur à 0',
  'common.active': 'Actif',
  'common.deleted': 'Supprimé',
  'common.unknown': 'Inconnu',
  'common.backHome': 'Retour au tableau de bord',
  'common.export': 'Exporter',
  'common.import': 'Importer',
  'common.importSuccess': 'Import réussi',
  'common.importFail': 'Import échoué',
  'common.upload': 'Téléverser',
  'common.selected': 'sélectionné(s)',
  'common.bulkDeleteTitle': 'Supprimer les éléments sélectionnés',
  'common.bulkDeleteConfirm': 'Supprimer {{count}} éléments ? Cette action est irréversible.',
  'common.unsavedTitle': 'Modifications non sauvegardées',
  'common.unsavedMsg': 'Vous avez des modifications non sauvegardées. Les abandonner ?',
  'common.discard': 'Abandonner',
  'common.new': 'Nouveau',
  'common.refresh': 'Actualiser',
  'common.value': 'Valeur',
  'common.dash': '—',

  // Les traductions Système / API / Projet / Accueil / Manuel sont
  // livrées avec plugin-ui (voir ligoj-plugins/plugin-ui/ui/src/i18n) et
  // injectées dans ce store au moment de l'install() du plugin.

  // Dashboard
  'dashboard.welcome': 'Bienvenue, {{name}}',
  'dashboard.noModules': 'Aucun module disponible.',
  'dashboard.manageUsers': 'Gérer les comptes utilisateurs',
  'dashboard.manageGroups': 'Gérer les groupes',
  'dashboard.manageCompanies': 'Gérer les entités',
  'dashboard.manageDelegates': 'Gérer les délégations',
  'dashboard.manageContainerScopes': 'Gérer les portées de conteneurs',
  'dashboard.manageProjects': 'Gérer les projets et souscriptions',
  'dashboard.manageAdmin': 'Administration système',
  'dashboard.manageInformation': 'Voir l\'état du système',
  'dashboard.manageConfiguration': 'Gérer la configuration système',
  'dashboard.manageSystemUsers': 'Gérer les comptes système',
  'dashboard.manageRoles': 'Gérer les rôles et permissions',
  'dashboard.managePlugins': 'Gérer les plugins installés',
  'dashboard.manageNodes': 'Gérer les nœuds et fournisseurs IAM',
  'dashboard.manageCache': 'Gérer le cache applicatif',
  'dashboard.manageBench': 'Tester les performances de la base',

  // Subscriptions
  'subscription.title': 'Souscriptions',
  'subscription.add': 'Ajouter une souscription',
  'subscription.remove': 'Retirer la souscription',
  'subscription.removeConfirm': 'Retirer {{name}} de ce projet ?',
  'subscription.service': 'Service / Outil',
  'subscription.demo': 'Données de démo — Installez des plugins (JIRA, Jenkins, SonarQube, etc.) pour gérer de vraies souscriptions.',
  'subscription.empty': 'Aucune souscription. Installez des plugins et ajoutez des souscriptions pour connecter ce projet à vos outils.',
  'subscription.demoAdd': 'Mode démo — Installez des plugins pour ajouter de vraies souscriptions.',

  // Users
  'user.title': 'Utilisateurs',
  'user.new': 'Nouvel utilisateur',
  'user.edit': 'Modifier l\'utilisateur',
  'user.login': 'Identifiant',
  'user.loginHint': 'Identifiant unique, non modifiable après la création',
  'user.firstName': 'Prénom',
  'user.lastName': 'Nom',
  'user.company': 'Entité',
  'user.emails': 'Emails',
  'user.groups': 'Groupes',
  'user.locked': 'Statut',
  'user.deleteTitle': 'Supprimer l\'utilisateur',
  'user.deleteConfirm': 'Êtes-vous certain de supprimer {{id}} ?',
  'user.noProvider': 'Fournisseur d\'identité indisponible',
  'user.noProviderMsg': 'Aucun fournisseur d\'identité configuré. Connectez un plugin IAM (LDAP, AD, etc.) pour gérer les utilisateurs.',
  'user.demoMode': 'Données d\'identité en cache. Connectez un plugin IAM pour la gestion en temps réel.',
  'user.demoEdit': 'Mode démo — Aucun fournisseur d\'identité configuré. Connectez un plugin IAM (LDAP, AD, etc.) pour gérer les utilisateurs.',
  'user.demoDelete': 'Mode démo — Connectez un plugin IAM pour supprimer des utilisateurs',
  'user.demoSave': 'Mode démo — Connectez un plugin IAM pour enregistrer les modifications',

  // Groups
  'group.title': 'Groupes',
  'group.new': 'Nouveau groupe',
  'group.edit': 'Modifier le groupe',
  'group.detailsTitle': 'Détails du groupe',
  'group.scope': 'Portée',
  'group.members': 'Membres',
  'group.locked': 'Verrouillé',
  'group.deleteTitle': 'Supprimer le groupe',
  'group.deleteConfirm': 'Êtes-vous certain de supprimer {{name}} ?',
  'group.noProvider': 'Aucun fournisseur d\'identité configuré. Connectez un plugin IAM (LDAP, AD, etc.) pour gérer les groupes.',
  'group.demoEdit': 'Mode démo — Aucun fournisseur d\'identité configuré. Connectez un plugin IAM (LDAP, AD, etc.) pour gérer les groupes.',
  'group.demoDelete': 'Mode démo — Connectez un plugin IAM pour supprimer des groupes',
  'group.demoSave': 'Mode démo — Connectez un plugin IAM pour enregistrer les modifications',

  // Companies
  'company.title': 'Entités',
  'company.new': 'Nouvelle entité',
  'company.edit': 'Modifier l\'entité',
  'company.detailsTitle': 'Détails de l\'entité',
  'company.deleteTitle': 'Supprimer l\'entité',
  'company.deleteConfirm': 'Êtes-vous certain de supprimer {{name}} ?',
  'company.noProvider': 'Aucun fournisseur d\'identité configuré. Connectez un plugin IAM (LDAP, AD, etc.) pour gérer les entités.',
  'company.demoEdit': 'Mode démo — Aucun fournisseur d\'identité configuré. Connectez un plugin IAM (LDAP, AD, etc.) pour gérer les entités.',
  'company.demoDelete': 'Mode démo — Connectez un plugin IAM pour supprimer des entités',
  'company.demoSave': 'Mode démo — Connectez un plugin IAM pour enregistrer les modifications',

  // Delegates
  'delegate.title': 'Délégués',
  'delegate.new': 'Nouveau délégué',
  'delegate.edit': 'Modifier le délégué',
  'delegate.receiver': 'Destinataire',
  'delegate.receiverType': 'Type de destinataire',
  'delegate.type': 'Type',
  'delegate.resource': 'Ressource',
  'delegate.resourceHint': 'Motif de ressource (ex: ou=.*)',
  'delegate.admin': 'Admin',
  'delegate.write': 'Écriture',
  'delegate.deleteTitle': 'Supprimer le délégué',
  'delegate.deleteConfirm': 'Êtes-vous certain de supprimer la délégation pour {{name}} ?',
  'delegate.empty': 'Aucun délégué configuré.',

  // Administration
  'admin.title': 'Administration système',
  'admin.health': 'État du système',
  'admin.version': 'Version API',
  'admin.pluginCount': 'Plugins installés',
  'admin.plugins': 'Plugins',
  'admin.key': 'Clé',
  'admin.versionCol': 'Version',
  'admin.type': 'Type',
  'admin.vendor': 'Éditeur',
  'admin.nodes': 'Nœuds',
  'admin.subscriptions': 'Souscriptions',
  'admin.config': 'Configuration',
  'admin.configFilter': 'Filtrer la configuration',
  'admin.value': 'Valeur',
  'admin.source': 'Source',
  'admin.persisted': 'Persisté',
  'admin.noPlugins': 'Aucun plugin installé',
  'admin.noConfig': 'Aucune entrée de configuration',

  // Profile
  'profile.title': 'Profil',
  'profile.account': 'Compte',
  'profile.username': 'Nom d\'utilisateur',
  'profile.roles': 'Rôles',
  'profile.administrator': 'Administrateur',
  'profile.adminTooltip': 'Vous disposez des privilèges d\'administrateur — accès complet à toutes les fonctionnalités et réglages.',
  'profile.permissions': 'Permissions',
  'profile.uiAuth': 'Autorisations UI',
  'profile.apiAuth': 'Autorisations API',
  'profile.preferences': 'Préférences',
  'profile.apiTokens': 'Clés API',
  'profile.apiTokensHint': 'Clés API',
  'profile.apiVerify': 'Vérifier',
  'profile.apiVerifyTitle': 'Vérification des accès API',
  'profile.apiVerifyAuths': '{n} autorisations',
  'profile.apiVerifyRate': '{allowed} opérations autorisées sur {total}',
  'profile.apiVerifyFilterPlaceholder': 'URL ou chemin à tester — ex. https://hote/ligoj/rest/project ou rest/project',
  'profile.apiVerifyAllowed': 'Autorisé par vos autorisations : {methods}',
  'profile.apiVerifyDenied': 'Aucune autorisation API ne correspond à cette URL',
  'profile.apiVerifyNoOps': "Autorisé par vos autorisations, mais aucune opération API ne correspond à cette URL",
  'profile.apiVerifyMethod': 'Méthode',
  'profile.apiVerifyPath': 'Chemin',
  'profile.apiVerifyDesc': 'Description',
  'profile.apiVerifyStatus': 'Statut',
  'profile.apiVerifyStatusAllowed': 'Autorisé par vos autorisations',
  'profile.apiVerifyStatusDenied': 'Non autorisé par vos autorisations',
  'profile.authentication': "Authentification",
  'profile.authProvider': "Fournisseur d’authentification",
  'profile.authProviderInternal': "Comptes internes",
  'profile.authProviderHint': "Nœud du fournisseur d’identité principal : {node}",
  'profile.authProviderInternalHint': "Aucun nœud de fournisseur d’identité configuré (feature:iam:node:primary) : comptes internes.",
  'profile.lastAuthentication': "Dernière authentification",
  'profile.lastAuthenticationUnknown': "Inconnue",
  'profile.mfaDevices': "Appareils multi-facteurs",
  'profile.mfaAdd': "Ajouter un appareil",
  'profile.mfaNone': "Aucun appareil enregistré : votre mot de passe (ou votre fournisseur d’identité) suffit pour vous connecter. Enregistrez une application d’authentification pour exiger un code après chaque authentification, OIDC compris.",
  'profile.mfaUnavailable': "L’authentification multi-facteurs n’est pas disponible sur ce serveur.",
  'profile.mfaHint': "Demandé après chaque connexion. Gardez un appareil accessible, ou retirez-les tous.",
  'profile.mfaDeviceMeta': "{type} · enregistré le {created} · dernière utilisation {lastUsed}",
  'profile.mfaNeverUsed': "jamais",
  'profile.mfaRemove': "Retirer",
  'profile.mfaRemoveTitle': "Retirer cet appareil ?",
  'profile.mfaRemoveText': "« {name} » ne sera plus accepté. Sans aucun appareil, aucun code n’est demandé à la connexion.",
  'profile.mfaAddTitle': "Enregistrer une application d’authentification",
  'profile.mfaName': "Nom de l’appareil",
  'profile.mfaNameHint': "ex. Téléphone, Portable",
  'profile.mfaScan': "Scannez ce QR code avec votre application d’authentification (Google Authenticator, Microsoft Authenticator, Aegis…), ou saisissez le secret, puis entrez le code affiché :",
  'profile.mfaCode': "Code affiché par l’application",
  'profile.mfaCodeHint': "6 chiffres",
  'profile.mfaRegister': "Enregistrer",
  'profile.mfaInvalidCode': "Code invalide : vérifiez l’heure de votre appareil et réessayez",
  'profile.mfaNameTaken': "Un appareil porte déjà ce nom",
  'profile.mfaType.TOTP': "Application d’authentification",
  'profile.mfaType.PASSKEY': "Passkey",
  'profile.mfaMethodPasskey': "Passkey",
  'profile.mfaMethodPasskeyHint': "Touch ID, Windows Hello, une clé de sécurité… : le navigateur vous sollicite à la vérification",
  'profile.mfaPasskeyUnsupported': "Ce navigateur ne prend pas en charge les passkeys",
  'profile.mfaPasskeyIntro': "Nommez l’appareil, puis votre navigateur vous demande de créer la passkey (empreinte, visage, code ou clé de sécurité). La passkey est liée à ce site.",
  'profile.mfaPasskeyRegister': "Créer la passkey",
  'profile.mfaPasskeyInvalid': "La passkey n’a pas pu être créée ou a été refusée : réessayez",
  'profile.mfaKind.app': "Application d’authentification",
  'profile.mfaKind.securityKey': "Clé de sécurité",
  'profile.mfaKind.phone': "Passkey sur téléphone",
  'profile.mfaKind.platform': "Authentificateur intégré",
  'profile.mfaKind.passkey': "Passkey",
  'profile.mfaTransport.usb': "USB",
  'profile.mfaTransport.nfc': "NFC",
  'profile.mfaTransport.ble': "Bluetooth",
  'profile.mfaTransport.smart-card': "carte à puce",
  'profile.mfaTransport.internal': "intégré",
  'profile.mfaTransport.hybrid': "téléphone (QR code)",
  'profile.mfaManage': "Gérer",
  'profile.mfaSummary': "{count} enregistré(s) · par défaut : {name}",
  'profile.mfaDefault': "Par défaut",
  'profile.mfaSetDefault': "Utiliser par défaut à la vérification",
  'profile.mfaMethodTotp': "Application d’authentification (TOTP)",
  'profile.mfaMethodTotpHint': "Google Authenticator, Microsoft Authenticator, Aegis… : un code à 6 chiffres renouvelé toutes les 30 secondes",
  'profile.apiVerifyError': "Impossible de charger la description de l'API",
  'profile.apiVerifyRowTooltip': "Ouvrir cette opération dans l'explorateur d'API (nouvel onglet)",
  'profile.theme': 'Thème',
  'profile.compact': 'Mode compact',
  'profile.compactHint': 'Espacements resserrés, polices plus petites, tableaux plus denses',
  'profile.reduceMotion': 'Réduire les animations',
  'profile.reduceMotionHint': 'Désactive les effets ripple, les animations de dialogs et menus, et la barre de progression',
  'profile.demoMode': 'Mode démo',
  'demo.indicator': 'Démo',
  'demo.indicatorTip': 'Le mode démo est actif : du contenu de démonstration (outils, projets et pages vitrines fictifs) est ajouté aux vues de ce navigateur uniquement. Cliquez pour ouvrir votre profil et le désactiver.',
  'profile.tipLogin': 'Identifiant',
  'profile.tipMail': 'Mail',
  'profile.tipRoles': 'Rôles',
  'profile.demoModeHint': 'Ajoute du contenu de démonstration aux vues : outils fictifs sur le tableau de bord, projets fictifs et une section démo dans le dialogue projet',
  'profile.demoModeTooltip': 'Réservé aux administrateurs. Le contenu de démonstration n\'est stocké nulle part : il complète uniquement l\'affichage de ce navigateur, et la préférence est mémorisée localement.',
  'profile.skipUnsavedConfirmation': 'Quitter sans confirmation',
  'profile.skipUnsavedConfirmationHint': 'Quitte les formulaires modifiés sans demander confirmation',
  'profile.language': 'Langue',

  // About
  'about.reportBug': 'Signaler un bug',
  'about.reportBugHint': 'Modèle pré-rempli + formulaire d\'issue GitHub',

  // Bug report dialog

  // Login
  'login.title': 'Ligoj',
  'login.username': 'Nom d\'utilisateur',
  'login.password': 'Mot de passe',
  'login.submit': 'Se connecter',
  'login.failed': 'Identifiant ou mot de passe invalide',
  'login.error': 'Échec de la connexion',
  'login.networkError': 'Erreur réseau. Veuillez réessayer.',

  // Les chaînes « notification.* » sont fournies par plugin-inbox-sql —
  // un déploiement sans ce plugin n'embarque pas des textes qu'il ne
  // pourrait pas afficher. Voir ligoj-plugins/plugin-inbox-sql/ui/src/i18n.

  // Errors & 404
  'error.notFound': 'Page introuvable',
  'error.notFoundMsg': 'La page n\'existe pas ou nécessite un plugin qui n\'est pas installé.',
  'error.pluginFailed': 'Échec du chargement du plugin',

  // Catalogue d'erreurs serveur (porté depuis `error.mod.js` /
  // `nls/error-messages.js`). Voir le commentaire de `en.js` pour la
  // rationale du namespace plat.
  'error.business': 'Erreur métier',
  'error.technical': 'Erreur technique gérée',
  'error.internal': 'Erreur technique',
  'error.unknownCode': 'Une erreur technique est survenue, veuillez contacter le support',
  'error.0': 'Mode hors ligne, aucun serveur métier disponible',
  'error.400': 'Erreur de validation',
  'error.401': 'Authentification requise',
  'error.401-details': 'Votre session a expiré, vous devez vous reconnecter',
  'error.403': 'Autorisation requise',
  'error.403-details': 'Vous n\'avez pas les autorisations pour effectuer cette action',
  'error.404': 'Service métier non disponible',
  'error.404-details': "Le service métier '{this}' demandé n'existe pas. Rafraîchissez le cache de votre navigateur ou contactez votre administrateur",
  'error.404-data-title': 'Données non disponibles',
  'error.404-data': "La donnée '{this}' demandée n'existe pas",
  'error.404-entity': "L'objet métier '{this}' demandé n'existe pas",
  'error.412': 'Intégrité des données',
  'error.412-unknown-details': 'Impossible d\'effectuer cette opération en raison d\'un problème inconnu d\'intégrité des données',
  'error.412-foreign-details': 'Impossible d\'effectuer cette opération sur {to} à cause d\'une relation depuis {from}',
  'error.412-unicity-details': 'Impossible d\'effectuer cette opération à cause d\'une entrée en doublon {entry} ({name})',
  'error.415': 'Type de média invalide',
  'error.415-details': 'Le contrat du service appelé a peut-être changé',
  'error.500': 'Erreur interne',
  'error.501': 'Pas encore disponible',
  'error.501-details': 'Cette erreur gérée concerne une fonctionnalité non encore disponible. Réessayez plus tard ou contactez votre administrateur',
  'error.503': 'Ressource serveur',
  'error.503-details': "Le service métier '{this}' demandé est actuellement indisponible. Réessayez plus tard ou contactez votre administrateur",
  'error.504': 'Délai de communication dépassé',
  'error.504-details': 'Le serveur métier semble surchargé, réessayez plus tard',
  'error.status': 'Statut',
  'error.message': 'Message',
  'error.business-down': 'Le serveur métier n\'est pas disponible',
  'error.database-down': 'La base de données n\'est pas disponible',
  'error.database-lock': 'La base de données a un verrou en cours sur l\'enregistrement que vous tentez de mettre à jour. Réessayez plus tard ou contactez votre administrateur',
  'error.ldap-down': 'L\'annuaire utilisateur n\'est pas disponible',
  'error.mail-down': 'Le mail n\'a pas pu être envoyé',
  'error.cycle': 'Cycle {this} détecté',
  // Règles de validation (cf. en.js pour la convention).
  'error.rule.NotNull': 'Ce champ est requis',
  'error.rule.NotBlank': 'Ce champ est requis',
  'error.rule.NotEmpty': 'Ce champ est requis',
  'error.rule.Size': 'Taille invalide',
  'error.rule.Min': 'Valeur trop petite',
  'error.rule.Max': 'Valeur trop grande',
  'error.rule.Pattern': 'Format invalide',
  'error.rule.Email': 'Adresse email invalide',
  'error.rule.Positive': 'Doit être positif',
  'error.rule.Mapping': 'Type invalide',
  'error.rule.length': 'Longueur invalide',
  'error.rule.already-exist': 'Existe déjà',
  'error.rule.unknown-id': 'Identifiant inconnu',
  'error.rule.group-type': 'Le groupe sélectionné n\'est pas de type Projet',
  'error.rule.StartsWith': 'Doit commencer par {0}',

  // Common (additions)
  'common.confirm': 'Confirmer',
  'common.dontAskAgain': 'Ne plus me demander',

  // User actions
  'user.actions': 'Actions',
  'user.lock': 'Verrouiller',
  'user.unlock': 'Déverrouiller',
  'user.isolate': 'Isoler',
  'user.restore': 'Restaurer',
  'user.resetPassword': 'Réinitialiser le mot de passe',
  'user.lockConfirm': 'Verrouiller l\'utilisateur {{id}} ? Il ne pourra plus se connecter.',
  'user.unlockConfirm': 'Déverrouiller l\'utilisateur {{id}} ?',
  'user.isolateConfirm': 'Isoler l\'utilisateur {{id}} ? Cela supprimera toutes ses appartenances aux groupes.',
  'user.restoreConfirm': 'Restaurer l\'utilisateur {{id}} ? Cela rétablira ses appartenances précédentes aux groupes.',
  'user.resetPasswordConfirm': 'Réinitialiser le mot de passe de {{id}} ? Un mot de passe temporaire sera généré.',
  'user.demoAction': 'Mode démo — Connectez un plugin IAM pour effectuer cette action',

  // Container Scopes
  'nav.containerScopes': 'Portées de conteneurs',
  'containerScope.title': 'Portées de conteneurs',
  'containerScope.new': 'Nouvelle portée',
  'containerScope.edit': 'Modifier la portée',
  'containerScope.view': 'Voir la portée',
  'containerScope.locked': 'Verrouillé',
  'containerScope.lockedHint': 'Une fois verrouillée, cette portée ne pourra plus être modifiée ni supprimée — uniquement consultée.',
  'containerScope.deleteTitle': 'Supprimer la portée',
  'containerScope.deleteConfirm': 'Êtes-vous certain de supprimer {{name}} ?',
  'containerScope.noProvider': 'Aucun fournisseur d\'identité configuré.',
  'containerScope.demoMode': 'Données de démo. Connectez un plugin IAM pour la gestion en temps réel.',
  'containerScope.demoSave': 'Mode démo — Connectez un plugin IAM pour enregistrer les modifications',
  'containerScope.demoDelete': 'Mode démo — Connectez un plugin IAM pour supprimer des portées',

  // Agreement

  // Import progress
  'import.progress': 'Import en cours...',
  'import.entries': '{{count}} entrées traitées',
  'import.done': 'Import terminé avec succès',
  'import.failed': 'Import échoué',

  // Group parent
  'group.parent': 'Groupe parent',
  'group.parentHint': 'Groupe parent optionnel pour la hiérarchie',

  // Plugin: SCM / Git
  'service.scm': 'Gestion de sources',
  'service.scm.git': 'Git',
  'service.scm.git.url': 'URL du dépôt',
  'service.scm.git.repository': 'Dépôt',

  // Plugin: KM / Confluence
  'service.km': 'Connaissance',
  'service.km.confluence': 'Confluence',
  'service.km.confluence.space': 'Espace',
  'service.km.confluence.url': 'URL',
  'service.km.confluence.spaces': 'Espaces Confluence',
  'service.km.confluence.activity': 'Dernière activité',
  'service.km.confluence.authWarning': 'Vous devez être authentifié sur Confluence pour voir les espaces.',
  'service.km.confluence.login': 'S\'authentifier',
  'service.km.confluence.refresh': 'Recharger',

  // Plugin: Build / Jenkins
  'service.build': 'Construction',
  'service.build.jenkins': 'Jenkins',
  'service.build.jenkins.job': 'Tâche',
  'service.build.jenkins.build': 'Construire',
  'service.build.jenkins.building': 'En construction...',
  'service.build.jenkins.branches': 'Branches',
  'service.build.jenkins.success': 'Succès',
  'service.build.jenkins.unstable': 'Instable',
  'service.build.jenkins.failure': 'Échec',
  'service.build.jenkins.disabled': 'Désactivé',
  'service.build.jenkins.buildTriggered': 'Construction lancée avec succès',

  // Plugin: Bug Tracker / SLA
  'service.bt': 'Anomalies',
  'service.bt.sla': 'SLA',
  'service.bt.sla.name': 'Nom',
  'service.bt.sla.start': 'Démarrage',
  'service.bt.sla.stop': 'Arrêt',
  'service.bt.sla.pause': 'Pause',
  'service.bt.sla.threshold': 'Seuil',
  'service.bt.sla.priority': 'Priorité',
  'service.bt.businessHours': 'Heures ouvrées',

  // Plugin: Bug Tracker / JIRA
  'service.bt.jira': 'JIRA',
  'service.bt.jira.project': 'Projet JIRA',
  'service.bt.jira.issues': 'Anomalies par priorité',
  'service.bt.jira.import': 'Importer CSV',
  'service.bt.jira.importMode': 'Mode d\'import',
  'service.bt.jira.encoding': 'Encodage',
  'service.bt.jira.importSuccess': 'Import terminé avec succès',
  'service.bt.jira.importFailed': 'Import échoué',

  // Plugin: VM
  'service.vm': 'Machines virtuelles',
  'service.vm.status': 'Statut',
  'service.vm.poweredOn': 'Allumée',
  'service.vm.poweredOff': 'Éteinte',
  'service.vm.suspended': 'Suspendue',
  'service.vm.on': 'Démarrer',
  'service.vm.off': 'Éteindre',
  'service.vm.suspend': 'Suspendre',
  'service.vm.shutdown': 'Arrêter',
  'service.vm.reset': 'Réinitialiser',
  'service.vm.reboot': 'Redémarrer',
  'service.vm.schedules': 'Planification',
  'service.vm.snapshots': 'Instantanés',
  'service.vm.cron': 'Expression CRON',
  'service.vm.createSnapshot': 'Créer un instantané',

  // Plugin: Inbox / Messaging
  'inbox.title': 'Messages',
  'inbox.new': 'Nouveau message',
  'inbox.noMessages': 'Aucun message',
  'inbox.from': 'De',
  'inbox.to': 'À',
  'inbox.message': 'Message',
  'inbox.send': 'Envoyer',
  'inbox.targetType': 'Type de destinataire',
  'inbox.target': 'Destinataire',
  'inbox.audience': 'Audience',
  'inbox.noEditDelete': 'Les messages ne peuvent pas être modifiés ou supprimés après envoi.',
  'inbox.loadMore': 'Charger plus',
  'inbox.user': 'Utilisateur',
  'inbox.group': 'Groupe',
  'inbox.company': 'Entité',
  'inbox.project': 'Projet',
  'inbox.node': 'Outil',

  // Plugin: Provisioning
  'service.prov': 'Provisionnement',
  'service.prov.monthlyCost': 'Coût mensuel',
  'service.prov.resources': 'Ressources',
  'service.prov.noResources': 'Aucune ressource. Ajoutez-en pour estimer les coûts.',
  'service.prov.deleteConfirm': 'Supprimer la ressource {{name}} ?',
  'service.prov.instance': 'Instance',
  'service.prov.database': 'Base de données',
  'service.prov.container': 'Conteneur',
  'service.prov.function': 'Fonction',
  'service.prov.storage': 'Stockage',
  'service.prov.support': 'Support',
  'service.prov.cpu': 'CPU',
  'service.prov.ram': 'RAM',
  'service.prov.os': 'OS',
  'service.prov.engine': 'Moteur',
  'service.prov.size': 'Taille',
  'service.prov.optimized': 'Optimisé pour',
  'service.prov.level': 'Niveau de support',
  'service.prov.location': 'Emplacement',
  'service.prov.usage': 'Profil d\'utilisation',
  'service.prov.budget': 'Budget',
  'service.prov.license': 'Licence',
  'service.prov.term': 'Durée',
  'service.prov.quantity': 'Quantité',
  'service.prov.catalog': 'Catalogues',
  'service.prov.catalog.import': 'Importer le catalogue',
  'service.prov.catalog.location': 'Emplacement préféré',
  'service.prov.catalog.preferredLocation': 'Définir l\'emplacement préféré',
  'service.prov.currency': 'Devises',
  'service.prov.currency.unit': 'Symbole',
  'service.prov.currency.rate': 'Taux de change',
  'service.prov.currency.quotes': 'Devis',
  'service.prov.currency.deleteConfirm': 'Supprimer la devise {{name}} ?',
  'service.prov.terraform': 'Terraform',
  'service.prov.terraform.notInstalled': 'Terraform n\'est pas installé sur ce serveur.',
  'service.prov.terraform.generate': 'Générer',
  'service.prov.terraform.plan': 'Planifier',
  'service.prov.terraform.apply': 'Appliquer',
  'service.prov.terraform.destroy': 'Détruire',
  'service.prov.network': 'Réseau',
  'service.prov.network.inbound': 'Entrant',
  'service.prov.network.outbound': 'Sortant',
  'service.prov.network.peer': 'Pair',
  'service.prov.network.port': 'Port',
  'service.prov.network.rate': 'Débit',
  'service.prov.network.throughput': 'Bande passante',
  'service.prov.admin': 'Administration du provisionnement',

  // Common additions for plugins
  'common.open': 'Ouvrir',
  'common.copy': 'Copier',
  'common.copied': 'Copié !',
  'common.operation': 'Opération',
  'common.trigger': 'Déclencher',

  // Shared import-progress label
  'common.importing': "Import en cours…",

  // Shared audit/preview/table chrome
  'common.audit': "Audit",
  'common.auditTitle': "Données d'audit",
  'common.createdBy': "Créé par",
  'common.createdDate': "Créé le",
  'common.lastModifiedBy': "Modifié par",
  'common.lastModifiedDate': "Modifié le",
  'common.noData': "Aucune donnée",
  'common.demo': "démo",
  'common.reduce': "Réduire",
  'subscription.deleteConfirm': "Supprimer cette souscription ?",

  // Dynamic chrome keys (subscription.status.*, theme, notFound, pager)
  'subscription.status.ok': "actif",
  'subscription.status.warn': "instable",
  'subscription.status.err': "hors-ligne",
  'subscription.status.idle': "inactif",
  'common.detailsToggle': "Détails",
  'common.dismiss': "Fermer",
  'profile.themeLight': "Clair",
  'profile.themeDark': "Sombre",
  'notFound.title': "Page introuvable",
  'notFound.message': "La page demandée n'existe pas ou a été déplacée.",
  'notFound.back': "Retour à l'accueil",
  'common.rows': "Lignes",
  'common.of': "sur",
  'common.previous': "Précédent",
  'common.next': "Suivant",
  // table tools (LjDataTable cog menu)
  'common.exportCsv': "Exporter en CSV",
  'common.copyClipboard': "Copier dans le presse-papiers",
  'common.tableTools': "Outils du tableau",
}
