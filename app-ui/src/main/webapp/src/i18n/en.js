export default {
  // Navigation
  'nav.home': 'Home',
  'nav.dashboard': 'Dashboard',
  'nav.identity': 'Identity',
  'nav.users': 'Users',
  'nav.groups': 'Groups',
  'nav.companies': 'Companies',
  'nav.delegates': 'Delegates',
  'nav.projects': 'Projects',
  'nav.system': 'Administration',
  'nav.information': 'Information',
  'nav.configuration': 'Configuration',
  'nav.hooks': 'Hooks',
  'nav.roles': 'Roles',
  'nav.plugins': 'Plugins',
  'nav.nodes': 'Nodes',
  'nav.cache': 'Cache',
  'nav.bench': 'Bench',
  'nav.tasks': 'Tasks',
  'nav.systemUsers': 'System users',
  'nav.profile': 'Profile',
  'node.iconMissing': 'Plugin unavailable',
  'nav.about': 'About',
  'nav.logout': 'Logout',
  'nav.refresh': 'Refresh',
  'nav.api': 'API',
  'nav.apiExplorer': 'Explorer',
  'nav.apiTokens': 'API keys',

  // Common
  'common.search': 'Search',
  'common.save': 'Save',
  'common.create': 'Create',
  'common.cancel': 'Cancel',
  'common.delete': 'Delete',
  'common.remove': 'Remove',
  'common.edit': 'Edit',
  'common.view': 'View',
  'common.add': 'Add',
  'common.close': 'Close',
  'common.actions': 'Actions',
  'common.alreadyExists': 'Already exists',
  'common.name': 'Name',
  'common.description': 'Description',
  'common.status': 'Status',
  'common.loading': 'Loading...',
  'common.loadError': 'The request failed. Please retry.',
  'common.required': 'This field is required',
  'common.positive': 'Must be greater than 0',
  'common.active': 'Active',
  'common.deleted': 'Deleted',
  'common.unknown': 'Unknown',
  'common.backHome': 'Back to Dashboard',
  'common.export': 'Export',
  'common.import': 'Import',
  'common.importSuccess': 'Import succeeded',
  'common.importFail': 'Import failed',
  'common.upload': 'Upload',
  'common.selected': 'selected',
  'common.bulkDeleteTitle': 'Delete selected items',
  'common.bulkDeleteConfirm': 'Are you sure you want to delete {{count}} items? This cannot be undone.',
  'common.unsavedTitle': 'Unsaved changes',
  'common.unsavedMsg': 'You have unsaved changes. Discard them?',
  'common.discard': 'Discard',
  'common.new': 'New',
  'common.refresh': 'Refresh',
  'common.value': 'Value',
  'common.dash': '—',

  // System / API / Project / Home / Manual translations live with
  // plugin-ui (see ligoj-plugins/plugin-ui/ui/src/i18n) and are merged
  // into this store at plugin install() time.

  // Dashboard
  'dashboard.welcome': 'Welcome, {{name}}',
  'dashboard.noModules': 'No modules available.',
  'dashboard.manageUsers': 'Manage user accounts',
  'dashboard.manageGroups': 'Manage groups',
  'dashboard.manageCompanies': 'Manage companies',
  'dashboard.manageDelegates': 'Manage delegations',
  'dashboard.manageContainerScopes': 'Manage container scopes',
  'dashboard.manageProjects': 'Manage projects and subscriptions',
  'dashboard.manageAdmin': 'System administration',
  'dashboard.manageInformation': 'View system status',
  'dashboard.manageConfiguration': 'Manage system configuration',
  'dashboard.manageSystemUsers': 'Manage system accounts',
  'dashboard.manageRoles': 'Manage roles and permissions',
  'dashboard.managePlugins': 'Manage installed plugins',
  'dashboard.manageNodes': 'Manage nodes and IAM providers',
  'dashboard.manageCache': 'Manage application cache',
  'dashboard.manageBench': 'Test database performance',

  // Subscriptions
  'subscription.title': 'Subscriptions',
  'subscription.add': 'Add Subscription',
  'subscription.remove': 'Remove Subscription',
  'subscription.removeConfirm': 'Remove {{name}} from this project?',
  'subscription.service': 'Service / Tool',
  'subscription.demo': 'Demo data — Install plugins (JIRA, Jenkins, SonarQube, etc.) to manage real subscriptions.',
  'subscription.empty': 'No subscriptions. Install plugins and add subscriptions to connect this project to your tools.',
  'subscription.demoAdd': 'Demo mode — Install plugins to add real subscriptions.',

  // Users
  'user.title': 'Users',
  'user.new': 'New User',
  'user.edit': 'Edit User',
  'user.login': 'Login',
  'user.loginHint': 'Unique login, cannot be changed after creation',
  'user.firstName': 'First Name',
  'user.lastName': 'Last Name',
  'user.company': 'Company',
  'user.emails': 'Emails',
  'user.groups': 'Groups',
  'user.locked': 'Status',
  'user.deleteTitle': 'Delete User',
  'user.deleteConfirm': 'Are you sure you want to delete {{id}}?',
  'user.noProvider': 'Identity provider not available',
  'user.noProviderMsg': 'No identity provider is configured. Connect an IAM plugin (LDAP, AD, etc.) to manage users.',
  'user.demoMode': 'Showing cached identity data. Connect an IAM plugin for live management.',
  'user.demoEdit': 'Demo mode — No identity provider configured. Connect an IAM plugin (LDAP, AD, etc.) to manage users.',
  'user.demoDelete': 'Demo mode — Connect an IAM plugin to delete users',
  'user.demoSave': 'Demo mode — Connect an IAM plugin to save changes',

  // Groups
  'group.title': 'Groups',
  'group.new': 'New Group',
  'group.edit': 'Edit Group',
  'group.detailsTitle': 'Group details',
  'group.scope': 'Scope',
  'group.members': 'Members',
  'group.locked': 'Locked',
  'group.deleteTitle': 'Delete Group',
  'group.deleteConfirm': 'Are you sure you want to delete {{name}}?',
  'group.noProvider': 'No identity provider is configured. Connect an IAM plugin (LDAP, AD, etc.) to manage groups.',
  'group.demoEdit': 'Demo mode — No identity provider configured. Connect an IAM plugin (LDAP, AD, etc.) to manage groups.',
  'group.demoDelete': 'Demo mode — Connect an IAM plugin to delete groups',
  'group.demoSave': 'Demo mode — Connect an IAM plugin to save changes',

  // Companies
  'company.title': 'Companies',
  'company.new': 'New Company',
  'company.edit': 'Edit Company',
  'company.detailsTitle': 'Company details',
  'company.deleteTitle': 'Delete Company',
  'company.deleteConfirm': 'Are you sure you want to delete {{name}}?',
  'company.noProvider': 'No identity provider is configured. Connect an IAM plugin (LDAP, AD, etc.) to manage companies.',
  'company.demoEdit': 'Demo mode — No identity provider configured. Connect an IAM plugin (LDAP, AD, etc.) to manage companies.',
  'company.demoDelete': 'Demo mode — Connect an IAM plugin to delete companies',
  'company.demoSave': 'Demo mode — Connect an IAM plugin to save changes',

  // Delegates
  'delegate.title': 'Delegates',
  'delegate.new': 'New Delegate',
  'delegate.edit': 'Edit Delegate',
  'delegate.receiver': 'Receiver',
  'delegate.receiverType': 'Receiver Type',
  'delegate.type': 'Type',
  'delegate.resource': 'Resource',
  'delegate.resourceHint': 'Resource pattern (e.g. ou=.*)',
  'delegate.admin': 'Admin',
  'delegate.write': 'Write',
  'delegate.deleteTitle': 'Delete Delegate',
  'delegate.deleteConfirm': 'Are you sure you want to delete the delegation for {{name}}?',
  'delegate.empty': 'No delegates configured.',

  // Administration
  'admin.title': 'Administration',
  'admin.health': 'System Status',
  'admin.version': 'API Version',
  'admin.pluginCount': 'Installed Plugins',
  'admin.plugins': 'Plugins',
  'admin.key': 'Key',
  'admin.versionCol': 'Version',
  'admin.type': 'Type',
  'admin.vendor': 'Vendor',
  'admin.nodes': 'Nodes',
  'admin.subscriptions': 'Subscriptions',
  'admin.config': 'Configuration',
  'admin.configFilter': 'Filter configuration',
  'admin.value': 'Value',
  'admin.source': 'Source',
  'admin.persisted': 'Persisted',
  'admin.noPlugins': 'No plugins installed',
  'admin.noConfig': 'No configuration entries',

  // Profile
  'profile.title': 'Profile',
  'profile.account': 'Account',
  'profile.username': 'Username',
  'profile.roles': 'Roles',
  'profile.administrator': 'Administrator',
  'profile.adminTooltip': 'You have administrator privileges — full access to all features and settings.',
  'profile.permissions': 'Permissions',
  'profile.uiAuth': 'UI Authorizations',
  'profile.apiAuth': 'API Authorizations',
  'profile.preferences': 'Preferences',
  'profile.apiTokens': 'API keys',
  'profile.apiTokensHint': 'API keys',
  'profile.apiVerify': 'Verify',
  'profile.apiVerifyTitle': 'API access verification',
  'profile.apiVerifyAuths': '{n} authorizations',
  'profile.apiVerifyRate': '{allowed} of {total} operations allowed',
  'profile.apiVerifyFilterPlaceholder': 'URL or path to test — e.g. https://host/ligoj/rest/project or rest/project',
  'profile.apiVerifyAllowed': 'Allowed by your authorizations: {methods}',
  'profile.apiVerifyDenied': 'No API authorization matches this URL',
  'profile.apiVerifyNoOps': 'Allowed by your authorizations, but no API operation matches this URL',
  'profile.apiVerifyMethod': 'Method',
  'profile.apiVerifyPath': 'Path',
  'profile.apiVerifyDesc': 'Description',
  'profile.apiVerifyStatus': 'Status',
  'profile.apiVerifyStatusAllowed': 'Allowed by your authorizations',
  'profile.apiVerifyStatusDenied': 'Not allowed by your authorizations',
  'profile.authentication': "Authentication",
  'profile.authProvider': "Authentication provider",
  'profile.authProviderInternal': "Internal accounts",
  'profile.authProviderHint': "Primary identity provider node: {node}",
  'profile.authProviderInternalHint': "No identity provider node configured (feature:iam:node:primary): internal accounts.",
  'profile.lastAuthentication': "Last authentication",
  'profile.lastAuthenticationUnknown': "Unknown",
  'profile.mfaDevices': "Multi-factor devices",
  'profile.mfaAdd': "Add a device",
  'profile.mfaNone': "No device registered: your password (or your identity provider) is enough to sign in. Register an authenticator application to require a code after every authentication, OIDC included.",
  'profile.mfaUnavailable': "Multi-factor authentication is not available on this server.",
  'profile.mfaHint': "Asked after every sign-in. Keep one device reachable, or remove them all.",
  'profile.mfaDeviceMeta': "{type} · registered {created} · last used {lastUsed}",
  'profile.mfaNeverUsed': "never",
  'profile.mfaRemove': "Remove",
  'profile.mfaRemoveTitle': "Remove this device?",
  'profile.mfaRemoveText': "“{name}” will no longer be accepted. Without any device left, no code is asked at sign-in.",
  'profile.mfaAddTitle': "Register an authenticator",
  'profile.mfaName': "Device name",
  'profile.mfaNameHint': "e.g. Phone, Laptop",
  'profile.mfaScan': "Scan this QR code with your authenticator application (Google Authenticator, Microsoft Authenticator, Aegis…), or type the secret, then enter the code it shows:",
  'profile.mfaCode': "Code shown by the application",
  'profile.mfaCodeHint': "6 digits",
  'profile.mfaRegister': "Register",
  'profile.mfaInvalidCode': "Invalid code: check the time of your device and try again",
  'profile.mfaNameTaken': "A device with this name already exists",
  'profile.mfaType.TOTP': "Authenticator app",
  'profile.mfaType.PASSKEY': "Passkey",
  'profile.mfaMethodPasskey': "Passkey",
  'profile.mfaMethodPasskeyHint': "Touch ID, Windows Hello, a security key…: the browser prompts you at verification",
  'profile.mfaPasskeyUnsupported': "This browser does not support passkeys",
  'profile.mfaPasskeyIntro': "Name the device, then your browser asks you to create the passkey (fingerprint, face, PIN or security key). The passkey is bound to this site.",
  'profile.mfaPasskeyRegister': "Create the passkey",
  'profile.mfaPasskeyInvalid': "The passkey could not be created or was rejected: try again",
  'profile.mfaKind.app': "Authenticator app",
  'profile.mfaKind.securityKey': "Security key",
  'profile.mfaKind.phone': "Phone passkey",
  'profile.mfaKind.platform': "Built-in authenticator",
  'profile.mfaKind.passkey': "Passkey",
  'profile.mfaTransport.usb': "USB",
  'profile.mfaTransport.nfc': "NFC",
  'profile.mfaTransport.ble': "Bluetooth",
  'profile.mfaTransport.smart-card': "smart card",
  'profile.mfaTransport.internal': "built-in",
  'profile.mfaTransport.hybrid': "phone (QR code)",
  'profile.mfaManage': "Manage",
  'profile.mfaSummary': "{count} registered · default: {name}",
  'profile.mfaDefault': "Default",
  'profile.mfaSetDefault': "Use as default at verification",
  'profile.mfaMethodTotp': "Authenticator app (TOTP)",
  'profile.mfaMethodTotpHint': "Google Authenticator, Microsoft Authenticator, Aegis…: a 6-digit code renewed every 30 seconds",
  'profile.apiVerifyError': 'Unable to load the API description',
  'profile.apiVerifyRowTooltip': 'Open this operation in the API explorer (new tab)',
  'profile.theme': 'Theme',
  'profile.compact': 'Compact mode',
  'profile.compactHint': 'Tighter spacing, smaller fonts, denser tables',
  'profile.reduceMotion': 'Reduce motion',
  'profile.reduceMotionHint': 'Disable ripples, dialog and menu animations, and progress bar motion',
  'profile.demoMode': 'Demo mode',
  'demo.indicator': 'Demo',
  'demo.indicatorTip': 'Demo mode is enabled: demonstration content (sample tools, projects, showcase pages) is blended into the views of this browser only. Click to open your profile and turn it off.',
  'profile.tipLogin': 'Login',
  'profile.tipMail': 'Mail',
  'profile.tipRoles': 'Roles',
  'profile.demoModeHint': 'Blend demonstration content into the views: sample tools on the dashboard, sample projects and a demo section in the project dialog',
  'profile.demoModeTooltip': 'Administrator-only. The demo content is stored nowhere: it only augments what this browser displays, and is remembered locally.',
  'profile.skipUnsavedConfirmation': 'Skip leave confirmation',
  'profile.skipUnsavedConfirmationHint': 'Leave dirty forms without confirmation prompt',
  'profile.language': 'Language',

  // About
  'about.reportBug': 'Report a bug',
  'about.reportBugHint': 'Pre-filled template + GitHub issue form',

  // Bug report dialog

  // Login
  'login.title': 'Ligoj',
  'login.username': 'Username',
  'login.password': 'Password',
  'login.submit': 'Sign in',
  'login.failed': 'Invalid username or password',
  'login.error': 'Login failed',
  'login.networkError': 'Network error. Please try again.',

  // Notifications strings shipped by plugin-inbox-sql (`notification.*`)
  // — installs without the plugin don't carry strings they'd never
  // render. See ligoj-plugins/plugin-inbox-sql/ui/src/i18n.

  // Errors & 404
  'error.notFound': 'Page not found',
  'error.notFoundMsg': 'The page does not exist or requires a plugin that is not installed.',
  'error.pluginFailed': 'Failed to load plugin',

  // Backend-error catalog (ported verbatim from the legacy
  // `error.mod.js` / `nls/error-messages.js` bundle). Keys mirror the
  // legacy flat namespace (`error400`, `error404-details`, …) so any
  // payload using `{code: "…", message: "…", parameters: [...]}` from
  // the api resolves through `t(key)` directly. Mustache-style
  // `{{this}}` placeholders are normalised to vue-i18n `{this}` by the
  // host's i18n loader (see `plugins/i18n.js`); the same go for
  // numeric `{{[0]}}` → `{0}` and named `{{name}}` → `{name}`.
  'error.business': 'Business error',
  'error.technical': 'Managed technical error',
  'error.internal': 'Technical error',
  'error.unknownCode': 'A technical error has occurred, please contact the support team',
  'error.0': 'Offline mode, no business server available',
  'error.400': 'Validation error',
  'error.401': 'Authentication required',
  'error.401-details': 'Your session has expired, you have to login',
  'error.403': 'Authorization required',
  'error.403-details': 'You don\'t have the authorizations to perform this action',
  'error.404': 'Business service not available',
  'error.404-details': "The requested business service '{this}' does not exist. Refresh the cache of your browser or contact your administrator",
  'error.404-data-title': 'Data not available',
  'error.404-data': "The requested data '{this}' does not exist",
  'error.404-entity': "The requested business object '{this}' does not exist",
  'error.412': 'Data integrity',
  'error.412-unknown-details': 'Unable to proceed this operation because of an unknown data integrity issue',
  'error.412-foreign-details': 'Unable to proceed this operation on {to} because of relation from {from}',
  'error.412-unicity-details': 'Unable to proceed this operation because of duplicate entry {entry} ({name})',
  'error.415': 'Invalid sent media type',
  'error.415-details': 'The contract of called service may have changed',
  'error.500': 'Internal error',
  'error.501': 'Not yet available',
  'error.501-details': 'This managed error is related to a not yet available feature. Retry later or contact your administrator',
  'error.503': 'Server resource',
  'error.503-details': "The requested business service '{this}' is currently not available. Retry later or contact your administrator",
  'error.504': 'Timeout communication',
  'error.504-details': 'Business server seems to be overloaded, retry later',
  'error.status': 'Status',
  'error.message': 'Message',
  'error.business-down': 'The business server is not available',
  'error.database-down': 'The data base is not available',
  'error.database-lock': 'The data base has a pending lock on the record you are trying to update. Retry later or contact your administrator',
  'error.ldap-down': 'The user directory is not available',
  'error.mail-down': 'The mail could not be sent',
  'error.cycle': 'Detected {this} cycle',
  // Validation rules — surfaced when a JSR-303-shape body
  // `{ errors: { <field>: [{ rule, parameters }] } }` lands. Each entry
  // is `error.rule.<rule-name>` so a backend `rule: "group-type"`
  // resolves to `error.rule.group-type`. The plain rule name is the
  // tail fallback if no localised entry exists.
  'error.rule.NotNull': 'This field is required',
  'error.rule.NotBlank': 'This field is required',
  'error.rule.NotEmpty': 'This field is required',
  'error.rule.Size': 'Invalid size',
  'error.rule.Min': 'Value too small',
  'error.rule.Max': 'Value too large',
  'error.rule.Pattern': 'Invalid format',
  'error.rule.Email': 'Invalid email',
  'error.rule.Positive': 'Must be positive',
  'error.rule.Mapping': 'Invalid type',
  'error.rule.length': 'Invalid length',
  'error.rule.already-exist': 'Already exists',
  'error.rule.unknown-id': 'Unknown identifier',
  'error.rule.group-type': 'The selected group is not of type Project',
  'error.rule.StartsWith': 'Must start with {0}',

  // Common (additions)
  'common.confirm': 'Confirm',
  'common.dontAskAgain': "Don't ask again",

  // User actions
  'user.actions': 'Actions',
  'user.lock': 'Lock',
  'user.unlock': 'Unlock',
  'user.isolate': 'Isolate',
  'user.restore': 'Restore',
  'user.resetPassword': 'Reset Password',
  'user.lockConfirm': 'Lock user {{id}}? They will no longer be able to sign in.',
  'user.unlockConfirm': 'Unlock user {{id}}?',
  'user.isolateConfirm': 'Isolate user {{id}}? This will remove all group memberships.',
  'user.restoreConfirm': 'Restore user {{id}}? This will re-add previous group memberships.',
  'user.resetPasswordConfirm': 'Reset the password for {{id}}? A new temporary password will be generated.',
  'user.demoAction': 'Demo mode — Connect an IAM plugin to perform this action',

  // Container Scopes
  'nav.containerScopes': 'Container Scopes',
  'containerScope.title': 'Container Scopes',
  'containerScope.new': 'New Scope',
  'containerScope.edit': 'Edit Scope',
  'containerScope.view': 'View Scope',
  'containerScope.locked': 'Locked',
  'containerScope.lockedHint': 'Once locked, this scope can no longer be edited or deleted — only viewed.',
  'containerScope.deleteTitle': 'Delete Scope',
  'containerScope.deleteConfirm': 'Are you sure you want to delete {{name}}?',
  'containerScope.noProvider': 'No identity provider is configured.',
  'containerScope.demoMode': 'Showing demo data. Connect an IAM plugin for live management.',
  'containerScope.demoSave': 'Demo mode — Connect an IAM plugin to save changes',
  'containerScope.demoDelete': 'Demo mode — Connect an IAM plugin to delete scopes',

  // Agreement

  // Import progress
  'import.progress': 'Import in progress...',
  'import.entries': '{{count}} entries processed',
  'import.done': 'Import completed successfully',
  'import.failed': 'Import failed',

  // Group parent
  'group.parent': 'Parent Group',
  'group.parentHint': 'Optional parent group for hierarchy',

  // Plugin: SCM / Git
  'service.scm': 'Source Control',
  'service.scm.git': 'Git',
  'service.scm.git.url': 'Repository URL',
  'service.scm.git.repository': 'Repository',

  // Plugin: KM / Confluence
  'service.km': 'Knowledge',
  'service.km.confluence': 'Confluence',
  'service.km.confluence.space': 'Space',
  'service.km.confluence.url': 'URL',
  'service.km.confluence.spaces': 'Confluence Spaces',
  'service.km.confluence.activity': 'Last Activity',
  'service.km.confluence.authWarning': 'You must be authenticated to Confluence to view spaces.',
  'service.km.confluence.login': 'Authenticate',
  'service.km.confluence.refresh': 'Refresh',

  // Plugin: Build / Jenkins
  'service.build': 'Build',
  'service.build.jenkins': 'Jenkins',
  'service.build.jenkins.job': 'Job',
  'service.build.jenkins.build': 'Build',
  'service.build.jenkins.building': 'Building...',
  'service.build.jenkins.branches': 'Branches',
  'service.build.jenkins.success': 'Success',
  'service.build.jenkins.unstable': 'Unstable',
  'service.build.jenkins.failure': 'Failure',
  'service.build.jenkins.disabled': 'Disabled',
  'service.build.jenkins.buildTriggered': 'Build triggered successfully',

  // Plugin: Bug Tracker / SLA
  'service.bt': 'Bug Tracker',
  'service.bt.sla': 'SLA',
  'service.bt.sla.name': 'Name',
  'service.bt.sla.start': 'Start',
  'service.bt.sla.stop': 'Stop',
  'service.bt.sla.pause': 'Pause',
  'service.bt.sla.threshold': 'Threshold',
  'service.bt.sla.priority': 'Priority',
  'service.bt.businessHours': 'Business Hours',

  // Plugin: Bug Tracker / JIRA
  'service.bt.jira': 'JIRA',
  'service.bt.jira.project': 'JIRA Project',
  'service.bt.jira.issues': 'Issues by Priority',
  'service.bt.jira.import': 'Import CSV',
  'service.bt.jira.importMode': 'Import Mode',
  'service.bt.jira.encoding': 'Encoding',
  'service.bt.jira.importSuccess': 'Import completed successfully',
  'service.bt.jira.importFailed': 'Import failed',

  // Plugin: VM
  'service.vm': 'Virtual Machines',
  'service.vm.status': 'Status',
  'service.vm.poweredOn': 'Powered On',
  'service.vm.poweredOff': 'Powered Off',
  'service.vm.suspended': 'Suspended',
  'service.vm.on': 'Power On',
  'service.vm.off': 'Power Off',
  'service.vm.suspend': 'Suspend',
  'service.vm.shutdown': 'Shutdown',
  'service.vm.reset': 'Reset',
  'service.vm.reboot': 'Reboot',
  'service.vm.schedules': 'Schedules',
  'service.vm.snapshots': 'Snapshots',
  'service.vm.cron': 'CRON Expression',
  'service.vm.createSnapshot': 'Create Snapshot',

  // Plugin: Inbox / Messaging
  'inbox.title': 'Messages',
  'inbox.new': 'New Message',
  'inbox.noMessages': 'No messages',
  'inbox.from': 'From',
  'inbox.to': 'To',
  'inbox.message': 'Message',
  'inbox.send': 'Send',
  'inbox.targetType': 'Recipient Type',
  'inbox.target': 'Recipient',
  'inbox.audience': 'Audience',
  'inbox.noEditDelete': 'Messages cannot be edited or deleted after sending.',
  'inbox.loadMore': 'Load more',
  'inbox.user': 'User',
  'inbox.group': 'Group',
  'inbox.company': 'Company',
  'inbox.project': 'Project',
  'inbox.node': 'Tool',

  // Plugin: Provisioning
  'service.prov': 'Provisioning',
  'service.prov.monthlyCost': 'Monthly Cost',
  'service.prov.resources': 'Resources',
  'service.prov.noResources': 'No resources. Add one to start estimating costs.',
  'service.prov.deleteConfirm': 'Delete resource {{name}}?',
  'service.prov.instance': 'Instance',
  'service.prov.database': 'Database',
  'service.prov.container': 'Container',
  'service.prov.function': 'Function',
  'service.prov.storage': 'Storage',
  'service.prov.support': 'Support',
  'service.prov.cpu': 'CPU',
  'service.prov.ram': 'RAM',
  'service.prov.os': 'OS',
  'service.prov.engine': 'Engine',
  'service.prov.size': 'Size',
  'service.prov.optimized': 'Optimized for',
  'service.prov.level': 'Support Level',
  'service.prov.location': 'Location',
  'service.prov.usage': 'Usage Profile',
  'service.prov.budget': 'Budget',
  'service.prov.license': 'License',
  'service.prov.term': 'Term',
  'service.prov.quantity': 'Quantity',
  'service.prov.catalog': 'Catalogs',
  'service.prov.catalog.import': 'Import catalog',
  'service.prov.catalog.location': 'Preferred location',
  'service.prov.catalog.preferredLocation': 'Set preferred location',
  'service.prov.currency': 'Currencies',
  'service.prov.currency.unit': 'Symbol',
  'service.prov.currency.rate': 'Exchange Rate',
  'service.prov.currency.quotes': 'Quotes',
  'service.prov.currency.deleteConfirm': 'Delete currency {{name}}?',
  'service.prov.terraform': 'Terraform',
  'service.prov.terraform.notInstalled': 'Terraform is not installed on this server.',
  'service.prov.terraform.generate': 'Generate',
  'service.prov.terraform.plan': 'Plan',
  'service.prov.terraform.apply': 'Apply',
  'service.prov.terraform.destroy': 'Destroy',
  'service.prov.network': 'Network',
  'service.prov.network.inbound': 'Inbound',
  'service.prov.network.outbound': 'Outbound',
  'service.prov.network.peer': 'Peer',
  'service.prov.network.port': 'Port',
  'service.prov.network.rate': 'Rate',
  'service.prov.network.throughput': 'Throughput',
  'service.prov.admin': 'Provisioning Administration',

  // Common additions for plugins
  'common.open': 'Open',
  'common.copy': 'Copy',
  'common.copied': 'Copied!',
  'common.operation': 'Operation',
  'common.trigger': 'Trigger',

  // Shared import-progress label
  'common.importing': "Importing…",

  // Shared audit/preview/table chrome
  'common.audit': "Audit",
  'common.auditTitle': "Audit data",
  'common.createdBy': "Created by",
  'common.createdDate': "Created on",
  'common.lastModifiedBy': "Modified by",
  'common.lastModifiedDate': "Modified on",
  'common.noData': "No data",
  'common.demo': "demo",
  'common.reduce': "Collapse",
  'subscription.deleteConfirm': "Delete this subscription?",

  // Dynamic chrome keys (subscription.status.*, theme, notFound, pager)
  'subscription.status.ok': "up",
  'subscription.status.warn': "unstable",
  'subscription.status.err': "down",
  'subscription.status.idle': "idle",
  'common.detailsToggle': "Details",
  'common.dismiss': "Dismiss",
  'profile.themeLight': "Light",
  'profile.themeDark': "Dark",
  'notFound.title': "Page not found",
  'notFound.message': "The page you are looking for doesn't exist or has moved.",
  'notFound.back': "Back to home",
  'common.rows': "Rows",
  'common.of': "of",
  'common.previous': "Previous",
  'common.next': "Next",
  // table tools (LjDataTable cog menu)
  'common.exportCsv': "Export as CSV",
  'common.copyClipboard': "Copy to clipboard",
  'common.tableTools': "Table tools",
}
