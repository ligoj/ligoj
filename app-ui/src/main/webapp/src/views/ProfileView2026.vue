<!--
  ...
  Part of the UI 2026 redesign (feature/ui-2026). The <script setup> mirrors
  ProfileView.vue's logic (preset picker, compact toggle, skip-unsaved-
  confirmation, language) — same stores and plugins, identical behaviour.
  Only the presentation differs.

  Colours are derived from the ACTIVE Vuetify theme tokens (--v-theme-*) and
  icons use <v-icon> (currentColor), so the page follows whatever preset is
  selected (light OR dark) and never clashes with the shell. The "Vibrant"
  character comes from the layout: a gradient hero, a distinctive display
  font, soft rounded cards and primary-based accents.

  Reachable at /next/profile; the original /profile (ProfileView.vue) stays
...
onMounted(() => {
  // Lazy-load the Vibrant display font once (the main app bundle doesn't
  // ship it). Scoped to this redesign page.
  if (typeof document !== 'undefined' && !document.getElementById('ui2026-font')) {
...
<style scoped>
/* Colours pulled from the active Vuetify theme → adapts to any preset
   (light or dark) and never clashes with the shell. */
.p2026 {
...
.hero {
  ...
  padding: 24px 26px; margin-bottom: 20px; border-radius: 22px;
  background:
    radial-gradient(600px 200px at 100% -40%, rgba(255,255,255,.18), transparent 70%),
