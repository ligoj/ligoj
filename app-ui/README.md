...
```
java -Dligoj.endpoint="http://192.168.4.138:8081/ligoj-api" -jar target/app-ui-1.6.11.war
```

# Build Docker image
```
docker build -t ligoj-ui:1.6.11 --build-arg VERSION=1.6.11 .
```
...
```
docker build -t ligoj-ui:1.6.11 --build-arg VERSION=1.6.11 --build-arg WAR="https://oss.sonatype.org/service/local/repositories/orgligoj-1087/content/org/ligoj/app/app-ui/1.6.11/app-ui-1.6.11.war" .
```

Private remote build

```
docker build -t ligoj-ui:1.6.11 --build-arg VERSION=1.6.11 --build-arg WAR="https://storage.company.com/releases/app-ui-1.6.11.war" .
```

Reuse the local maven package

```
docker build -t ligoj-ui:1.6.11 --build-arg VERSION=1.6.11 --build-arg WAR="target/app-ui-1.6.11.war" .
```
...
docker run --rm -it \
 --name "ligoj-ui" \
 ligoj-ui:1.6.11 bash -c "apt-get install -y curl && curl --failed http://192.168.4.138:8081/ligoj-api/manage/health"
```
