...
```
java -Dligoj.endpoint="http://192.168.4.138:8081/ligoj-api" -jar target/app-ui-3.3.1.war
```

# Build Docker image

```
docker build -t ligoj/ligoj-ui:3.3.1 .
```
...
is "https://oss.sonatype.org/service/local/artifact/maven/redirect?r=public&g=org.ligoj.app&a=app-ui&v=3.3.1&p=war"
...
```bash
docker build -t ligoj/ligoj-ui:3.3.1 --progress=plain -f Dockerfile .
podman build -t ligoj/ligoj-ui:3.3.1 --progress=plain -f Dockerfile .
```

Also, compatible with `podman`, and multiple target architectures:

```bash
podman build --platform linux/arm64 --platform linux/amd64 --manifest ligoj/ligoj-ui -t ligoj/ligoj-ui:3.3.1 -f Dockerfile .
```
...
--name "ligoj-ui" \
--network="host" \
ligoj/ligoj-ui:3.3.1 sh -c "curl http://127.0.0.1:8088/ligoj-api/manage/health"
```
...
```
docker run -d --name ligoj-ui --link ligoj-api:api -p 8080:8080 ligoj/ligoj-ui:3.3.1 
```
