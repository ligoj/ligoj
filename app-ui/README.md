...
```
java -Dligoj.endpoint="http://192.168.4.138:8081/ligoj-api" -jar target/app-ui-3.1.1.war
```

# Build Docker image

```
docker build -t ligoj/ligoj-ui:3.1.1 .
```
...
```
docker build -t ligoj/ligoj-ui:3.1.1 --build-arg WAR="https://oss.sonatype.org/service/local/repositories/orgligoj-1087/content/org/ligoj/app/app-ui/3.1.0/app-ui-3.1.1.war" .
```

## Private remote build

```
docker build -t ligoj/ligoj-ui:3.1.1 --build-arg WAR="https://storage.company.com/releases/app-ui-3.1.1.war" .
```

## Local maven package

```
docker build -t ligoj/ligoj-ui:3.1.1 --build-arg WAR="target/app-ui-3.1.1.war" .
```
...
docker run --rm -it \
 --name "ligoj-ui" \
 ligoj/ligoj-ui:3.1.1 bash -c "apt-get install -y curl && curl --failed http://192.168.4.138:8081/ligoj-api/manage/health"
```
...
```
docker run -d --name ligoj-ui --link ligoj-api:api -p 8080:8080 ligoj/ligoj-ui:3.1.1 
```
