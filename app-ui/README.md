...
```
java -Dligoj.endpoint="http://192.168.4.138:8081/ligoj-api" -jar target/app-ui-3.0.8.war
```

# Build Docker image

```
docker build -t ligoj/ligoj-ui:3.0.8 .
```
...
By default, the location is "https://oss.sonatype.org/service/local/artifact/maven/redirect?r=public&g=org.ligoj.app&a=app-ui&v=3.0.8&p=war"
In case of a custom build you can specify its remote or local location.

## Staged OSS build from Sonatype

```
docker build -t ligoj/ligoj-ui:3.0.8 --build-arg WAR="https://oss.sonatype.org/service/local/repositories/orgligoj-1087/content/org/ligoj/app/app-ui/3.0.8/app-ui-3.0.8.war" .
```

## Private remote build

```
docker build -t ligoj/ligoj-ui:3.0.8 --build-arg WAR="https://storage.company.com/releases/app-ui-3.0.8.war" .
```

## Local maven package

```
docker build -t ligoj/ligoj-ui:3.0.8 --build-arg WAR="target/app-ui-3.0.8.war" .
```
...
docker run --rm -it \
 --name "ligoj-ui" \
 ligoj/ligoj-ui:3.0.8 bash -c "apt-get install -y curl && curl --failed http://192.168.4.138:8081/ligoj-api/manage/health"
```
...
```
docker run -d --name ligoj-ui --link ligoj-api:api -p 8080:8080 ligoj/ligoj-ui:3.0.8 
```
