...
- **Shared chrome components** (host, re-exported from `host.js`), used everywhere with the `.lj-surface` design-token class: `LjPageHeader` (title, subtitle, breadcrumb chips, `#actions`), `LjButton` (primary / ghost / danger), `LjSearch`, `LjDialog`, `LjSegmented` (tab toggle), `LjStatus` (semantic status chip), `LjAvailabilityField` (text field with a live "already exists" check). `.lj-surface` provides the `--ink / --pill / --radius / --mono / --surface / --card / --border*` variables. Theming axes on `<html>`: `data-style` (shadow / radius / border tokens shared by several presets), `data-preset` (exact preset id, scope CSS on it when two presets sharing a style must diverge), `data-reduce-motion`.
...
- **Pre-loaded** in `main.js`: `REQUIRED_PLUGINS = ['id', 'ui', 'prov']` (`plugins/eager-plugins.js`), awaited before `app.use(router)` and `mount`, so the routes and the sidebar menus they register exist on first paint. The boot fetches the session first and requests only the required plugins listed in `applicationSettings.data['ui-plugins']` (every one when an older backend omits the list, none without a session), so a deployment without plugin-prov never fetches `/main/prov/vue/index.js`.
...
- **Just-in-time** through `loadPlugin(id)`: the parameter dialogs' `ensureToolPluginLoaded(nodeId)` and `PluginFeatures` for an unloaded subscription plugin. The loader refuses, without any request, an id the session `ui-plugins` data does not list (`isPluginInstalled`): the subscriptions of a disabled or removed plugin stay listed, and their row delegation would otherwise request a bundle that 404s on every page.
...
- **Tables**: `LigojDataTable` / `LigojDataTableServer` (v-data-table + tools menu: CSV export, copy; forward `#header.<key>` / `#item.<key>` slots; header `tooltip` field), `LjDataTable` (presentation-only, caller keeps its `useDataTable` and listens to `@update:options`), `RowActionsMenu`, `ImportExportBar`. `LjDataTable` never pages its `items` itself: it renders what it receives and emits `update:options` (page, itemsPerPage, sortBy). Every caller must wire that event — server-side lists through `useDataTable` (rows / page / sidx / sord / search[value], `:fetch-all="dt.loadAll"` for exports; ProjectListView, SystemUserView…), in-memory lists through plugin-ui's `useClientTable(filteredRef, { defaultSort })` (`paged` / `total` / `onOptions`; SystemNodeView, ApiTokenView). Passing the full list with no listener leaves the pager and the sort inert.
- **Dialogs**: `LigojConfirmDialog` / `LjConfirmDialog` (same props / slots / events), `LjDialog` (title, optional `badge` = the edited entity identifier highlighted after the title, used by the user / group / company / delegate / project dialogs), `ApiVerifyDialog` (see "API explorer").
...
- **Nodes**: `NodeIcon` / `nodeIcon` (priority `uiClasses` → wrench for short ids → PNG; when both SVG and PNG 404 an inline "?" placeholder, `MISSING_NODE_ICON`; no request at all when `isToolAvailable(node)` is false, i.e. the node has `enabled: false` or its tool is absent from the session `plugins`, which also applies to the SVG colour lookup of `resolveToolColor`), `NodeModeChip`, `nodeType`, `isInstance`, `nodePluginId` (`service:prov:aws` → `'prov'`).
...
- **MFA devices**: `deviceKind` / `deviceIcon` / `deviceTransports` (`utils/mfaDevice.js`) — a passkey's kind from the `transports` + `attachment` the API returns per device (security key: usb/nfc/ble/smart-card or cross-platform; built-in: internal or platform; phone: hybrid), the `model` from the AAGUID (bootstrap `AaguidRegistry`, `META-INF/mfa-aaguid.properties`) shown in the profile meta line; needs a bootstrap with the extended `MfaDeviceVo`. The profile's Authentication card shows one summary row (count, default device, `defaultDevice()`) with a Manage link opening the "Multi-factor devices" `LjDialog` (rows, star / delete actions, hint, Add in the footer stacking the registration dialog). **User identity**: `visualIdName` / `resolveVisualId` / `userVisualId` / `userFullName` / `userLabel` (`utils/visualId.js`) — the ONE rule for `service:id:visual-id-name` (login by default, `mail`, a plain attribute or `customAttributes.<x>`, login fallback); the auth store's top-right fallback and plugin-id's `visualIdValue` delegate to it, plugin-ui renders every user through it (project team leader pill + tooltip full name, project dialog candidates `visual — Full Name` with the login as value, project header, subscription status, audit trail). **Tool colour**: `svgBrandColor` / `fallbackToolColor` / `resolveToolColor` / `toolIconBase` (`utils/toolBrand.js`) — the brand colour read once from the tool's SVG icon (most saturated mid-lightness fill/stroke/stop colour, neutrals ignored), else a known-brand table then a palette hash; plugin-ui wraps it in the reactive `toolColor(node, name)` (`src/toolColor.js`) used by the plugin manager tile (brand-filled 36px square, 36px icon) and the home / project subscription glyphs. No per-view colour tables anymore.
...
- `SubscriptionsPanel.vue`: toolbar (cards / list toggle, optional search, collapse-all) over a grid of `SubscriptionGroupCard` or a flat `LjDataTable`. Props `groups`, `defaultView`, `searchable`, `collapsible`, `loading`, `cog`; emits `rowmenu`, `row-appear`; slots `#toolbar`, `#empty`.
...
- **Plugin manager** (`SystemPluginView`): the per-row cog (`RowActionsCog`, same pattern as the node view) holds the enable/disable action and the deletion; no dedicated "Enabled" column. The toggle (`pluginToggle.js`) calls `PUT rest/system/plugin/{artifact}/disable|enable`, which renames the jar (`*.jar.disabled`) so the plug-in class loader skips or loads it at the next restart, like install / uninstall (configuration kept; there is no persisted `enabled` on nodes, `PUT rest/node` rejects it). `LigojPluginVo` carries `disabled` and `loaded`; `pluginState()` derives active / disabled / disabling / enabling (restart required) / updating (a newer jar is staged in the plug-ins directory, shown as a cancellable chip) / pending / deleted, each with its tooltip. `LigojPluginVo.embedded` flags a plug-in loaded from the application WAR (nested location) without any jar in the plug-ins directory: an "Embedded" chip, no delete action and no toggle (the API refuses with `plugin-embedded`); dropping a newer jar in the directory overrides it at the next restart since the plug-ins class-loader is child-first for classes. Three KPI cards (`pluginStats.js`) with stacked multi-color bars and per-segment tooltips, no legend: plug-ins by type, active plug-ins, verified plug-ins. The **Automation** dialog (`PluginAutomationDialog`, `rest/system/plugin/schedule`, app-api `PluginScheduleResource` with its own scheduler) drives `ligoj.plugin.check` + `.check.cron` (scheduled version check, Spring cron edited with `@vue-js-cron/vuetify`, format `spring`, numeric weekdays), `ligoj.plugin.update` (automatic download, only while the check is enabled, guarded by the risks warning dialog) and `ligoj.plugin.maintenance` + `.maintenance.cron` (restart only when an update is staged). The check result (`ligoj.plugin.check.last/.updates`) is decorated into the session as `plugin-updates`; "Check versions" runs `POST …/schedule/check` and refreshes the session.
- **System users** (`SystemUserView`, `GET rest/system/user/roles`): each account shows its assigned roles and, in a distinct chip with the granting group in its tooltip, its **federated roles**, the system roles named after one of the user's identity-provider groups (same rule as the login grant in `UserOrgResource#getGrantedAuthorities`: exact, upper- or lower-case name). They come from `SystemUserVo.federatedRoles`, filled by plugin-core `IamSystemUserDetailsProvider` from the IAM cache memberships; the edit dialog lists them read-only and "Verify" unions their authorizations.
...
- **Read-only attributes** (plugin-id): `service:id:read-only-attributes`, a node parameter of the identity service (`firstName`, `lastName`, `company`, `department`, `localId`, `mail`, `customAttributes.<name>`), is forwarded by `UserOrgResource#decorate` in the session data; `plugin-id/ui/src/readOnlyAttributes.js` (`isReadOnly(name, isEdit)`) makes the matching `UserEditDialog` fields read-only in edit mode with the `user.readOnlyHint` hint, and `UserOrgResource#update` refuses a changed value with a `read-only` validation error on the attribute.
...
- `service:id:visual-id-name` / `-label`: the attribute shown as the user identifier in tables (`id`, `firstName`, `lastName`, `mail`, `customAttributes.<x>`) with an optional static header label; resolved by plugin-id `ui/src/visualId.js` (always falling back to the login), used by `UserListView` and `GroupMembersPanel` first column, sorted on the `visual-id` key mapped server-side (`getOrderedColumns()`, `CustomAttributeComparator` in LDAP). The user dialog titles its edit mode with the visual id and edits the provider's custom attributes: the names come from the `service:id:custom-attributes` session data (comma separated, decorated by `UserOrgResource` from `IUserRepository#getCustomAttributes`) merged with the attributes present on the loaded user, one text field each, sent back in `customAttributes` (blank = `null`, which removes the attribute).
...
## Deprecated plugin parameters

`Parameter.deprecated` (plugin-core, nullable, CSV column `deprecated` optional) reaches `ParameterVo.deprecated`. plugin-ui `NodeEditDialog` and `SubscribeWizardView` keep the field editable, add the `pfield--deprecated` style (warning left border, "Deprecated" chip) and show, as hint, the plugin's `<parameter id>-deprecated` i18n key when it exists (`deprecationNotice()` in `utils/pluginParams.js`), else `wizard.params.deprecatedNotice`. A flagged definition needs plugin-core ≥ 5.0.2 at runtime: the CSV importer rejects a column the entity does not have (no shipped plugin uses the column yet; plugin-id-ldap removed its superseded parameter instead).
