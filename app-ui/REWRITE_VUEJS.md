...
- **Displayed username (`service:id:user-display`)**: plugin-id's session decoration ships the IAM `userDetails` (first/last name, mails, `customAttributes`) plus the `service:id:user-display` configuration; the host auth store resolves the app-bar username from it (`auth.displayName`): `id` (default), `mail`, `mail-short`, or any attribute name incl. custom attributes — always falling back to the login. The user button's tooltip is the identity card: full name, login, mails, custom attributes, roles and the administrator mention.
...
- **Never use bare `<v-autocomplete>` / `<v-select>`** — use the host's `LigojAutocomplete` / `LigojSelect` (see §4). They suppress the browser's native autofill overlay and honor reduce-motion for the dropdown; a bare Vuetify select regresses both.
